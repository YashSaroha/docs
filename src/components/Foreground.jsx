    ...
    <div className="fixed top-0 left-0 z-[3] w-full h-screen p-5 overflow-y-auto overflow-x-hidden">
      <div className="flex items-center justify-between w-1/3 h-[7%] bg-zinc-600 rounded-full px-2 mx-auto my-[3%]">
        <input
          ...
          className="w-[90%] h-full rounded-full outline-none pl-2 bg-transparent text-white tracking-wide"
        />
        <span
          onClick={handleSave}
        ...
        {todoArray.map((item) => {
          return (
            <motion.div
              ...
              className="relative flex-shrink-0 w-40 h-56 rounded-[40px] bg-zinc-900/90 text-white px-5 py-6 overflow-hidden"
            >
              <FaRegFileAlt />
              <p className="text-sm leading-tight mt-5 text-gray-300">
                {item.todo}
              </p>
